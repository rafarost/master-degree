...
\title{Decrypting Viginere Cipher}
%
%
...
\begin{abstract}
%\boldmath
In order to complete a coursework of master degree cryptography classes, in this report I have 
presented how to decrypt a given cipher-text that was encrypted using the Vigenere Cipher. 
As part of the coursework, a decrypt tool was developed in Ruby language. The tool usage and 
also the cryptanalysis method was explained step by step across the report.
\end{abstract}
...
\begin{IEEEkeywords}
Polyalphabetic Cipher, Vigenere Cipher, Index of Coincidence IC, Ruby.
\end{IEEEkeywords}
...
Suppose that we want to cipher the message "Hello World" with the key "QWER". We get the first plain text letter "H" and the fist key letter "Q" and look at the Viginere Square. The fist letter of the cipher-text will be the intersection between those two letters, in this case X. We keep doing the same with following plain text letters, restarting the key when needed, and we will get the "xapce ssibz" cipher-text. 

\section{Cryptanalysis of Vigenere Cipher}
...
Cipher text: cpimvtcmngiheycgremhwltmvtceiiqwivwhmumhwptawgoptfizftgitlezpcgxfspsbuchhbqwhalhdsa
...
\lstinputlisting[language=Ruby, firstline=104, lastline=128]{find-key.rb}
...
\lstinputlisting[language=Ruby, firstline=18, lastline=28]{find-key.rb}
...
On the second block, P was the most frequent letter, which means a shift of 11 letters from E. And so, the second key letter was L. On the third block, we noticed that E was the most frequent letter, and in this case, there was no shift, and the key letter must be A. Doing the same with all the blocks, we will get the entire key PLATO.

With the key and cipher-text in hands, we can now have access to the full plain-text, by simple applying the shift in each cipher-text position according the key.

And then we finally have the plain-text! "neither must we forget that the republic is but the third part of a still larger design which was to have included an ideal history of at". Having the plain-text the main goal of the coursework was achieved.
